...
- **Cut a release by pushing the tag.** With `main` green, push an annotated `vX.Y.Z` tag; the
  `release.yml` workflow runs `make check` on Ubuntu and macOS plus CLI/whisper smoke, builds the
  wheel and sdist, and creates a GitHub Release with those assets. It then waits for approval on the
  `pypi` environment and publishes to PyPI (see Publishing to PyPI below).
