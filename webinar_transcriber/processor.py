...
def process_input(
    ...
    *,
    ...
    def on_structure_progress(next_completed: float, *, detail: str | None = None) -> None:
        nonlocal completed
        advance = max(0.0, next_completed - completed)
        if advance > 0:
            active_reporter.progress_advanced("structure", advance=advance, detail=detail)
