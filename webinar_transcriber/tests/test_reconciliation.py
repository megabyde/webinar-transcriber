...
class TestReconcileDecodedWindows:
    ...
    def test_fixes_non_monotonic_boundaries_without_dropping_text(self) -> None:
        transcription = reconcile_decoded_windows([
            DecodedWindow(
                window=InferenceWindow(id="window-1", region_index=0, start_sec=0.0, end_sec=5.0),
                segments=[
                    TranscriptSegment(id="w1s1", text="Agenda review", start_sec=0.0, end_sec=3.5)
                ],
            ),
            DecodedWindow(
                window=InferenceWindow(id="window-2", region_index=0, start_sec=3.0, end_sec=7.0),
                segments=[
    ...
    def test_removes_duplicate_text_from_overlapping_window_boundary(self) -> None:
        transcription = reconcile_decoded_windows([
            DecodedWindow(
                window=InferenceWindow(id="window-1", region_index=0, start_sec=0.0, end_sec=28.0),
                segments=[
                    TranscriptSegment(
                        id="w1s1",
            ...
            DecodedWindow(
                window=InferenceWindow(id="window-2", region_index=0, start_sec=26.0, end_sec=54.0),
                segments=[
                    TranscriptSegment(
                        id="w2s1",
