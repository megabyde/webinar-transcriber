...
def test_warn_and_interrupted_render_messages() -> None:
    console = Console(record=True, width=100)
    reporter = RichStageReporter(console=console)
...
def test_count_and_rate_helpers_hide_empty_values() -> None:
...
def test_progress_advanced_ignores_inactive_or_mismatched_stage_keys() -> None:
...
def test_stage_finished_without_matching_active_event_reports_zero_elapsed() -> None:
    console = Console(record=True, width=100)
    reporter = RichStageReporter(console=console)
